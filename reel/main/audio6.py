...
for i in range(1, n):
    out = "x%d" % i
    fc.append("[%s][%d:a]acrossfade=d=%.3f:c1=tri:c2=nofade[%s]" % (cur, i, XF, out))
    cur = out
fc.append("[%s]loudnorm=I=-14:TP=-1.5:LRA=11[out]" % cur)
args += ["-filter_complex", ";".join(fc), "-map", "[out]",
         "-c:a", "pcm_s16le", "-ar", "48000", "-ac", "2",
         os.path.join(HERE, "audio6.wav")]
subprocess.run(args, check=True)
d = float(subprocess.run(["ffprobe", "-v", "error", "-show_entries", "format=duration",
    "-of", "csv=p=0", os.path.join(HERE, "audio6.wav")],
    capture_output=True, text=True).stdout)
print("\naudio6.wav %.4fs   picture %.4fs   delta %+.4f" % (d, m["total"], d - m["total"]))
