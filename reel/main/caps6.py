...
# What the transcriber got wrong, checked against the budget document:
# "ממלא מקום ראש העיר" is one person (slide 11 names his office), the council
# member is נגה, and "עדכון התקציב" is what was passed.
...
for seg in tr:
    for w in seg["words"]:
        ...
        if o is not None:
            ...
            raw.append((FIX.get(w["w"].strip(), w["w"].strip()),
                        round(o, 3), round(o + (w["e"] - w["s"]), 3), bi))
...
# The five lines each act lands on get the emphasis style: 104px, flush right
# against the white rule. BRAND.md allows five, and these are the five the reel
# is built to arrive at.
BUTTONS = ("קריסה.", "עם בורות ליד בתי הספר.", "הם ורק הם.",
           "פחות פיקוח, פחות ניקיון ופחות שירות לתושבים.",
           "מוסרית וגם פוליטית.")
tk = []
for g in groups:
    a = [w for w, _, _ in g]; t = [x for _, x, _ in g]; n = len(a); h = (n + 1) // 2
    tk.append({"start": round(max(0.0, t[0] - 0.06), 3), "end": 0.0,
               "style": "emphasis" if " ".join(a) in BUTTONS else "regular",
