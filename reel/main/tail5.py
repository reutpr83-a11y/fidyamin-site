...
# same grade beats5.py cuts its segments with
...
START, DUR = 156.823, 0.85        # picks up exactly where the last beat ended
Z, ANCH = 1.0667, 0.35
...
enc = subprocess.Popen(["ffmpeg", "-y", "-v", "error", "-f", "rawvideo", "-pix_fmt", "rgb24",
    ...
    "-colorspace", "bt709", os.path.join(HERE, "tail5.mp4")], stdin=subprocess.PIPE)
