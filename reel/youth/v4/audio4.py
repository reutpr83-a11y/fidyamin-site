...
m["durs"][-1] += TAIL
m["total"] += TAIL
XF = m["xf"]; segd = os.path.join(HERE, "s4")

for i, ((a, b), d) in enumerate(zip(m["beats"], m["durs"])):
    w = os.path.join(segd, "a%02d.wav" % i)
    subprocess.run(["ffmpeg", "-y", "-v", "error", "-ss", "%.5f" % a, "-t", "%.5f" % d,
...
args += ["-filter_complex", ";".join(fc), "-map", "[out]",
         ...
         os.path.join(HERE, "audio4.wav")]
subprocess.run(args, check=True)
d = float(subprocess.run(["ffprobe", "-v", "error", "-show_entries", "format=duration",
    "-of", "csv=p=0", os.path.join(HERE, "audio4.wav")],
    capture_output=True, text=True).stdout)
print("\naudio4.wav %.4fs   picture %.4fs   delta %+.4f" % (d, m["total"], d - m["total"]))
