...
She stops speaking at 80.84 in the take and holds; the reel needs that hold
so the card can dissolve in on silence rather than over "פשוט". These frames
...
# same crop and grade beats4.py cuts its segments with
...
START, DUR = 81.34, 0.80          # picks up exactly where beat 10 ended
Z, ANCH = 1.12, 0.42
...
enc = subprocess.Popen(["ffmpeg", "-y", "-v", "error", "-f", "rawvideo", "-pix_fmt", "rgb24",
    ...
    "-colorspace", "bt709", os.path.join(HERE, "tail4.mp4")], stdin=subprocess.PIPE)
