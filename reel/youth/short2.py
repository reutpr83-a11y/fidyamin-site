...
GRADE = ("eq=contrast=0.95:saturation=0.90:gamma=1.03,"
         ...
         "unsharp=5:5:0.4")
BEATS = json.load(open(os.path.join(HERE, "beats2.json")))
segd = os.path.join(HERE, "s2"); os.makedirs(segd, exist_ok=True)
durs = []
...
for tag,ext,out in (("v","mp4","plate2.mp4"),("a","wav","plate2a.wav")):
    lst=os.path.join(HERE,tag+"2_list.txt")
